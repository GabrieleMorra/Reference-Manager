...
def create_reference(topic_id, title, doi='', authors='', abstract='', notes='', citation_count=0, publication_year=None):
    """Create a new reference"""
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute(
        '''INSERT INTO paper_references (topic_id, title, doi, authors, abstract, notes, citation_count, publication_year)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?)''',
        (topic_id, title, doi, authors, abstract, notes, citation_count, publication_year)
    )
...
def update_reference(reference_id, title, doi='', authors='', abstract='', notes='', citation_count=0, publication_year=None):
    """Update a reference"""
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute(
        '''UPDATE paper_references
           SET title = ?, doi = ?, authors = ?, abstract = ?, notes = ?, citation_count = ?, publication_year = ?
           WHERE id = ?''',
        (title, doi, authors, abstract, notes, citation_count, publication_year, reference_id)
    )
    conn.commit()
    conn.close()
    return True
...
def duplicate_reference(reference_id, target_topic_id):
    ...
    cursor.execute(
        ...
        (target_topic_id, original['title'], original['doi'], original['authors'],
         original['abstract'], original['notes'], original['citation_count'], original['publication_year'])
    )
